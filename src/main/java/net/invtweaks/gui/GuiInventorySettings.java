...
public class GuiInventorySettings extends Screen {
	...
	private ButtonWidget sortingKeybind;
	private boolean selectSortedKey = false;
	...
	public void init() {
		...
		controlList.add(sortingKeybind = new ButtonWidget(99, p.getX() + 55, this.height / 6 + 72, "Sort Keys: " + Keyboard.getKeyName(Const.SORT_KEY_BINDING.keyCode)));
		...
		if(this.b.isMultiplayer()) {
			sortOnPickupBtn.active = false;
			sortOnPickupBtn.message = "Sort on pickup: Only in SP";
	...
	public void render(int i, int j, float f) {
		...
		this.drawCenteredString(this.textRenderer, "InventoryTweaks settings", this.width / 2, 20, 0xFFFFFF);
		super.render(i, j, f);
	}
	...
	protected void buttonClicked(ButtonWidget guibutton) {
		...
		case 4:
			...
			break;
		case 5:
			this.b.openScreen(new GuiShortcutsHelp(this.b, this, this.config));
			break;
		case 99:
			if(!selectSortedKey) {
				selectSortedKey = true;
				sortingKeybind.message = "> " + "Sort Keys: " + Keyboard.getKeyName(Const.SORT_KEY_BINDING.keyCode) + " <";
			}
			break;
		case 100:
			try {
		...
		case 102:
			...
			break;
		case 200:
			this.b.openScreen(this.parentScreen);
		}
	...
	private void toggleBooleanButton(ButtonWidget guibutton, String property, String label) {
		...
		guibutton.message = this.computeBooleanButtonLabel(property, label);
	}

	protected void keyPressed(char chr, int key) {
		if (this.selectSortedKey) {
			Const.SORT_KEY_BINDING.keyCode = key;
			sortingKeybind.message = "Sort Keys: " + Keyboard.getKeyName(Const.SORT_KEY_BINDING.keyCode);
			InvTweaks.getInstance().cfgManager.getConfig().setProperty("sortedKey", Keyboard.getKeyName(Const.SORT_KEY_BINDING.keyCode));
			selectSortedKey = false;
		} else {
			super.keyPressed((char)chr, key);
		}
	}
