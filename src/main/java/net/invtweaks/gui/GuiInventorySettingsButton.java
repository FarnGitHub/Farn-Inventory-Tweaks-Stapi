...
public class GuiInventorySettingsButton extends GuiIconButton {
	...
	public void render(Minecraft minecraft, int i, int j) {
		...
		if(this.active) {
			this.drawCenteredString(minecraft.textRenderer, this.message, this.x + 5, this.y - 1, this.getTextColor(i, j));
		}
	}
	...
	public boolean isMouseOver(Minecraft minecraft, int i, int j) {
		...
		if(!super.isMouseOver(minecraft, i, j)) {
			...
			minecraft.openScreen(new GuiInventorySettings(minecraft, Obfuscation.getCurrentScreenStatic(minecraft), config));
			return true;
		}
	}
}
