...
public class GuiShortcutsHelp extends Screen {
	...
	public void render(int i, int j, float f) {
		...
		this.drawCenteredString(this.textRenderer, "Shortcuts help", this.width / 2, 20, 0xFFFFFF);
		int y = this.height / 6;
		...
		y += 12;
		this.drawShortcutLine("Select sorting configuration", "0-9 + " + Keyboard.getKeyName(Const.SORT_KEY_BINDING.keyCode), 16746496, y);
		y += 12;
		super.render(i, j, f);
	}

	private void drawShortcutLine(String label, String value, int color, int y) {
		this.drawString(this.textRenderer, label, 50, y, -1);
		this.drawString(this.textRenderer, value.contains("DEFAULT") ? "-" : value, this.width / 2 + 40, y, color);
	}
