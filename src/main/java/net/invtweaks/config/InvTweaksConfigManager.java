	...
	private boolean extractFile(String resource, String destination) {
			...
			if(file20 != null && file20.length > 0) {
				...
				for(int i$ = 0; i$ < len$; ++i$) {
					...
					try {
						...
						InvTweaks.getInstance().log.warn("Failed to extract " + resource + " from mod: " + exception16.getMessage());
					}
				}
			}
		}
		...
		if(!resourceContents.isEmpty()) {
			try {
				...
				InvTweaks.getInstance().log.error("Cannot create " + destination + " file: " + iOException15.getMessage());
				return false;
			}
		} else {
			InvTweaks.logInGameStatic("The mod won\'t work, because " + resource + " could not be found!");
			InvTweaks.getInstance().log.error("Cannot create " + destination + " file: " + resource + " not found");
			return false;
		}
	}
