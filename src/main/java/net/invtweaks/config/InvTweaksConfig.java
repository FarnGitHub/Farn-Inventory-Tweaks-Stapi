...
public class InvTweaksConfig {
			...
			for(int ruleset = 0; ruleset < i$; ++ruleset) {
				...
				if(line.matches("^[\\w]*\\:$")) {
					if(!defaultRuleset || !defaultRulesetEmpty) {
						activeRuleset.finalizeInventory();
	...
	private void reset() {
		...
		this.properties.put("sortedKey", Keyboard.getKeyName(Keyboard.KEY_R));
	...
	private void loadProperties() throws IOException {
		...
		if(configPropsFile != null) {
			...
			Const.SORT_KEY_BINDING.keyCode = Keyboard.getKeyIndex(this.properties.getProperty("sortedKey", Keyboard.getKeyName(Keyboard.KEY_R)));
			fis.close();
		}
