...
import net.invtweaks.mixin.ContainerScreenAccessor;
import net.invtweaks.mixin.ScreenAccessor;
...
public class InvTweaks extends Obfuscation {
	public static final Logger log = LogManager.getLogger("InvTweaks");
	...
	public final void onSortingKeyPressed() {
		synchronized(this) {
			if(this.cfgManager.makeSureConfigurationIsLoaded()) {
				...
				if(guiScreen == null || guiScreen instanceof InventoryMenuScreen) {
				...
				try {
					...
					do {
						...
						for(int prefferedPositions = 0; prefferedPositions < 9; ++prefferedPositions) {
							...
							if(tree != null && tree.popAnimationTime == 5 && this.hotbarClone[prefferedPositions] == null) {
	...
	public static boolean getIsMouseOverSlot(InventoryMenuScreen guiContainer, InventorySlot slot, int i, int j) {
	...
	private boolean onTick() {
		...
		if(config == null) {
			...
			if(currentScreen == null || currentScreen instanceof SurvivalInventoryScreen) {
				this.cloneHotbar();
			}
		...
		if(Mouse.isButtonDown(2)) {
			...
			if(config.getProperty("enableMiddleClick").equals("true") && !this.chestAlgorithmButtonDown) {
				...
				if(this.isChestOrDispenser(guiScreen)) {
					InventoryMenuScreen guiContainer = (InventoryMenuScreen)guiScreen;
					InventoryMenu container = this.getContainer((InventoryMenuScreen) guiScreen);
					int slotCount = this.getSlots(container).size();
					int mouseX = Mouse.getEventX() * guiContainer.width / this.mc.width;
					int mouseY = guiContainer.height - Mouse.getEventY() * guiContainer.height / this.mc.height - 1;
					int target = 0;

					for(int timestamp = 0; timestamp < slotCount; ++timestamp) {
						InventorySlot slot = this.getSlot(container, timestamp);
	...
	private void handleGUILayout(Screen guiScreen) {
		...
		if(isContainer || guiScreen instanceof SurvivalInventoryScreen || guiScreen.getClass().getSimpleName().equals("GuiInventoryMoreSlots")) {
			byte w = 10;
			byte h = 10;
			...
			if(!customButtonsAdded) {
				if(!isContainer) {
					...
					InventoryMenuScreen guiContainer12 = (InventoryMenuScreen)guiScreen;
					int i13 = 54696386;
	...
	private void handleShortcuts(Screen guiScreen) {
		if(guiScreen instanceof InventoryMenuScreen && !guiScreen.getClass().getSimpleName().equals("MLGuiChestBuilding")) {
			if(!Mouse.isButtonDown(0) && !Mouse.isButtonDown(1)) {
				this.mouseWasDown = false;
			} else if(!this.mouseWasDown) {
				this.mouseWasDown = true;
				if(this.cfgManager.getConfig().getProperty("enableShortcuts").equals("true")) {
					this.cfgManager.getShortcutsHandler().handleShortcut((InventoryMenuScreen)guiScreen);
				}
			}
