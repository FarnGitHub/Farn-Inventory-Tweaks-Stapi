...
public class ItemTree {
	...
	public int getKeywordDepth(String keyword) {
		try {
			...
			InvTweaks.getInstance().log.error("The root category is missing: " + nullPointerException3.getMessage());
			return 0;
		}
	}
	...
	public int getKeywordOrder(String keyword) {
		...
		if(items != null && items.size() != 0) {
			...
			try {
				...
				InvTweaks.getInstance().log.error("The root category is missing: " + nullPointerException4.getMessage());
				return -1;
			}
		}
	}
	...
	private void log(ItemTreeCategory category, int indentLevel) {
		...
		InvTweaks.getInstance().log.info(logIdent + category.getName());
		...
		while(iterator8.hasNext()) {
			...
			while(i$1.hasNext()) {
				ItemTreeItem item = (ItemTreeItem)i$1.next();
				InvTweaks.getInstance().log.info(logIdent + "  " + item + " " + item.getId() + " " + item.getDamage());
			}
		}
