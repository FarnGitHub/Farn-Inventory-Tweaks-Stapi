...
public class ShortcutsHandler extends Obfuscation {
		...
		while(true) {
			...
			do {
				while(true) {
					...
					int i11 = this.mc.options.forwardKey.keyCode;
					int i12 = this.mc.options.backKey.keyCode;
	...
	public void handleShortcut(InventoryMenuScreen guiScreen) {
		this.updateKeyStatuses();
		int ex = Mouse.getEventX();
		int ey = Mouse.getEventY();
		int x = ex * guiScreen.width / this.mc.width;
		int y = guiScreen.height - ey * guiScreen.height / this.mc.height - 1;
		boolean shortcutValid = false;
		InventorySlot slot = this.getSlotAtPosition(guiScreen, x, y);
	...
	private void move(boolean separateStacks, boolean drop) throws Exception {
		...
		synchronized(this) {
			...
			if(toIndex1 != -1) {
				...
				case 1:
					for(InventorySlot i$1 = this.container.getSlot(this.fromSection, this.fromIndex); i$1.hasStack() && toIndex1 != -1; toIndex1 = this.getNextIndex(separateStacks, drop)) {
	...
	private void craftAll(boolean separateStacks, boolean drop) throws Exception {
		...
		InventorySlot slot = this.container.getSlot(this.fromSection, this.fromIndex);
		if(slot.hasStack()) {
	...
	private boolean haveControlsChanged() {
		return !this.shortcutKeysStatus.containsKey(this.mc.options.forwardKey.keyCode) || !this.shortcutKeysStatus.containsKey(this.mc.options.backKey.keyCode);
	}
			...
			if(!emptySlotOnly) {
				...
				for(Iterator i$ = this.container.getSlots(this.toSection).iterator(); i$.hasNext(); ++i) {
					...
					if(slot.hasStack()) {
						...
						if(stack.equals(this.fromStack) && this.getStackSize(stack) < this.getMaxStackSize(stack)) {
							result = i;
							break;
						}
					}
	...
	private InventorySlot getSlotAtPosition(InventoryMenuScreen guiContainer, int i, int j) {
		for(int k = 0; k < guiContainer.menu.slots.size(); ++k) {
			InventorySlot slot = (InventorySlot)guiContainer.menu.slots.get(k);
