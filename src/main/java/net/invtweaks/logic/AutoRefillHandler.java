			...
			while(true) {
				...
				do {
					if(!i$.hasNext()) {
						...
						if(replacementStack != null) {
							InvTweaks.getInstance().log.info("Automatic stack replacement.");
							(new Thread((new Runnable() {
								...
								public void run() {
									if(AutoRefillHandler.this.isMultiplayerWorld()) {
										...
										if(e >= 1500) {
											InvTweaks.getInstance().log.warn("Autoreplace timout");
										}
									} else {
									...
									try {
										...
										if(itemStack5 != null && AutoRefillHandler.this.getItemID(itemStack5) == this.expectedItemId) {
											if(this.containerMgr.move(this.i, this.targetedSlot)) {
												...
												InvTweaks.getInstance().log.warn("Failed to move stack for autoreplace, despite of prior tests.");
											}
										}
									} catch (NullPointerException nullPointerException3) {
									} catch (TimeoutException timeoutException4) {
										InvTweaks.getInstance().log.error("Failed to trigger autoreplace: " + timeoutException4.getMessage());
									}
