...
public class SortingHandler extends Obfuscation {
	...
	public void sort() throws TimeoutException {
		...
		if(this.algorithm != 0) {
				...
				if(globalContainer.hasSection(ContainerManager.ContainerSection.CRAFTING_IN)) {
					...
					if(i != -1) {
						...
						while(rulePriority.hasNext()) {
							InventorySlot i1 = (InventorySlot)rulePriority.next();
							if(i1.hasStack()) {
						...
						if(!item19.isDamageable()) {
							...
							for(int i27 = 0; i27 < i25; ++i27) {
								...
								if(stackToMove.intValue() > 0) {
									...
									if(j != null && itemStack17.equals(j)) {
							...
							while(true) {
								...
								do {
									do {
										...
										preferredSlots = (InventorySlot)fromItems.next();
									} while(!preferredSlots.canSetStack(itemStack17));
								} while(preferredSlots.hasStack() && itemArmor21.slot <= ((ArmorItem)preferredSlots.getStack().getItem()).slot);
			...
			while(iterator16.hasNext()) {
				...
				if(InvTweaks.getInstance().log.isDebugEnabled()) {
					InvTweaks.getInstance().log.info("Rule : " + sortingRule18.getKeyword() + "(" + i20 + ")");
				}
		...
		if(InvTweaks.getInstance().log.isDebugEnabled()) {
			...
			InvTweaks.getInstance().log.info("Sorting done in " + timer + "ns");
		}
	...
	private void defaultSorting() throws TimeoutException {
		...
		if(iterations == 50) {
			InvTweaks.getInstance().log.info("Sorting takes too long, aborting.");
		}
		...
		if(from != null && !this.frozenSlots[j] && !this.frozenSlots[i]) {
			if(this.lockPriorities[i] <= priority) {
				...
				if(to != null) {
					...
					if(!canBeSwappedOrMerged && from.equals(to) && this.getStackSize(to) < to.getMaxSize()) {
						canBeSwappedOrMerged = true;
					}
