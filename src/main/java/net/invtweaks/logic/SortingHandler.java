...
public class SortingHandler extends Obfuscation {
    ...
    public SortingHandler(InvTweaksConfig config,
                        ...
                        for (Slot craftingSlot : craftingSlots) {
                            if (craftingSlot.hasStack()) {
                                globalContainer.move(
                                        ...
                                        globalContainer.getSlotIndex(craftingSlot.id),
                ...
                for (int i = size - 1; i >= 0; i--) {
                    ...
                    if (from != null) {
                        ...
                        if (fromItem.isDamageable()) {
                             if (fromItem instanceof ArmorItem fromItemArmor) {
                                 List<Slot> armorSlots = globalContainer.getSlots(ContainerSection.ARMOR);
                                 for (Slot slot : armorSlots) {
                                     if (slot.canInsert(from)
                                             && (!slot.hasStack() || fromItemArmor.type >
                                                     ((ArmorItem) slot.getStack().getItem()).type)) {
                                         globalContainer.move(ContainerSection.INVENTORY, i,
                                                 ContainerSection.ARMOR, 
                                                 globalContainer.getSlotIndex(slot.id));
                                     }
                                 }
                             }
                        }
        ...
        if (lockPriorities[i] <= priority) {
            ...
            // Try to swap/merge
                ...
                if (!canBeSwappedOrMerged
                       && from.isItemEqual(to)
                       && getStackSize(to) < to.getMaxCount() ) {
                    canBeSwappedOrMerged = true;
                }
            ...
            if (keywordOrder[i] == keywordOrder[j]) {
                ...
                if (getItemID(iStack) == getItemID(jStack)) {
                    if (getStackSize(iStack) == getStackSize(jStack)) {
                        ...
                        return (damageDiff < 0 && !iStack.isDamageable()
                                || damageDiff > 0 && iStack.isDamageable());
                    } else {
